...
## §1 Purpose
anima-lite is the custodian of the alignment between what a codebase promises and what it actually is (`RESOLUTION.md`). It treats code as a structure of promises to a user, not a description of a system, and exists to stop divergence between promise and artifact — whether introduced in seconds (slop) or accrued over years (debt) — from landing unratified. Porting a feature from prototype to production is the first work-type this custody takes and the only one fully built end to end; ripple (instituting a promise in N repos at once) and debt-work (fixing a diagnosed divergence) are ratified direction, built partway.
