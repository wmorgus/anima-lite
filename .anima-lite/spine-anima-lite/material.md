...
## §2 Runtime
- Claude Code skill runner — `/ari-<name>` invocation loads and executes the matching `.claude/skills/<name>/SKILL.md` in the active agent context; six skills exist today (§9)
...
- Claude Code: `.claude/skills/{ari-intake,ari-map,ari-argue-rhetoric,ari-code-rhetoric,ari-backlog,ari-read}/SKILL.md` + `.claude/agents/ari-lite.md` + `.claude/hooks/session-cost.py`
...
- **work-types** (`intent.md`'s `Work-type:` field): **port** (prototype → prod, argument already exists in code), **ripple** (one promise instituted in N repos at once, no source leg — `reorient/ripple.md`), **debt-work** (a diagnosed divergence fix; diagnosis layer itself is ratified direction, not yet built), **harness-change** (the harness changing its own skills/gates/specs; single-spine posture, PIN-32/PIN-36)
