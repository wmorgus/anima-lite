...
4. **Promise→promise graph (deferred)** — real ripple machinery for PIN-26, once a genuine N-leg shared-origin feature exists to design against. Don't build ahead of a real case; the shape of the need isn't known yet.

## Recommendation

Ship (1) and (3) together as a single compact instruction change to `ari-map`. Hold (2) until (1) is live and at least one spine has been re-audited by hand, so the staleness heuristic is checked against a real case before automating it. Hold (4) entirely — it's the one item that would add real structural weight, and nothing today justifies the cost.
