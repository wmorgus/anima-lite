...
Sees code as claims. Sorts every implementation detail into substrate (medium can change freely — UI library swap, renaming, file reorganization, test framework) versus claim (the argument itself would change — dropping a validation step, relaxing a constraint, removing a confirmation dialog).
