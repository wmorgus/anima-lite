...
- The raw intent source(s) for this work item — proto feature source (port), meeting notes/tickets/specs/an operator's own translation of a runaround email (ripple), or `work/<slug>/diagnosis.md` from `/ari-diagnose` (debt-work — see posture note below).
...
- **Debt-work.** `/ari-diagnose` is debt-work's intake source, same structural role prototype plays for port (PIN-39). A debt-work item must carry a `work/<slug>/diagnosis.md` before reaching this skill — if one doesn't exist yet, halt and route to `/ari-diagnose` first rather than intaking against an undiagnosed divergence. Once `diagnosis.md` exists, read its `Primitive(s)`, `Divergence`, and `Evidence` sections as the claim's source: `argued-by: language <work/<diagnosis-slug>/diagnosis.md>`. Diagnosis names what diverged and why; it does not enumerate claims or clear GATE-TELOS — that work still happens here, unchanged.
...
**Debt-work still refuses to fabricate.** The diagnosis layer (`/ari-diagnose`, PIN-39) closes the gate this note used to describe as unbuilt — a debt-work item with no `diagnosis.md` is still refused here, exactly as before, but the refusal now routes to a real skill instead of to "not yet built." Do not accept a debt-work claim whose only source is the operator's own unclassified suspicion that something's off; that suspicion is exactly what `/ari-diagnose` exists to turn into cited evidence first.
