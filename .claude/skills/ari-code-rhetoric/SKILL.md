...
## Arete posture (arete work-type)

An arete work item's execution target is inverted relative to every other work-type this skill runs: the target is a **new, empty repo**, not the existing (context-starved) repo the arete statement was founded for. That old repo is read-only evidence throughout — `/ari-arete-pan`'s judgment, never a live edit target again. What's different, and what isn't:

- **No spine-proto/spine-prod split, and no single-repo split either.** There are, in effect, three repos in view at once: the arete statement's home (`.anima-lite/work/<arete-slug>/`, this repo), the old repo (`ari-map`'s spine of it is read for evidence, never a translation target), and the new repo (which does not exist until this skill's Step 2 creates it). Step 1's plan and Step 2's execution subagent read the contract (from `/ari-argue-rhetoric`'s arete posture), the ratified `arete-statement.md`, and `/ari-arete-pan`'s `cut.md` — never the old repo's own spine as a substrate-translation guide, because there is no substrate translation happening here; a nugget or ore claim is realized fresh in the new repo's own idiom, chosen by the execution subagent the same way any greenfield code choice would be, not carried over from the old repo's pattern.
- **The new repo's first commit is its `ARETE.md`.** Before any nugget or ore claim is implemented, the execution subagent initializes the new repo and commits the ratified arete statement (copied from `arete-statement.md`'s `## Statement` section, same minimal shape `RESOLUTION.md` uses: sentence + ratification line) as `ARETE.md` at the new repo's root. This is not itself a claim commit — it is the founding artifact landing in the place it will govern, and it happens before Step 2's ordinary per-claim commit loop begins.
- **Nuggets get a commit with no behavioral diff beyond re-creation.** A nugget claim confirmed by `/ari-argue-rhetoric` means "this code, largely as it stood, is written into the new repo" — the commit exists (per commit discipline, below) even though the content is a faithful re-creation rather than a change. State this plainly in that claim's commit message rather than letting an unusually thin diff look like an accident.
- **Ore claims are rewrites, not translations.** An ore claim's contract entry carries the pressure-note from `cut.md`; the execution subagent's job is to satisfy the *pressure* the note names, in a form that also serves the arete statement — not to port the old code's shape into the new repo with light edits. If the old form and the statement-serving form turn out to be nearly identical, that's a legitimate outcome, but it must be arrived at by writing toward the statement, not by starting from the old file and trimming.
- **Slag is simply absent.** It never reached the contract; there is nothing to skip or stub for it in the new repo.
- **No playwright verification, same as harness-change** — Step 3 D/E does not apply unless the new repo happens to stand up a reachable dev server mid-workstream, in which case treat it as an ordinary port's playwright pass from that point on, not an arete-specific case.
- **Reconcile targets the new repo, not this one.** Step 4's "prod repo" is the new repo created in Step 2 — its own feature-branch/PR-or-direct-commit conventions apply, entirely separate from this repo's own git history. This repo's `.anima-lite/work/<slug>/` artifacts (contract, plan, blips, cut, arete-statement) stay here and are never copied into the new repo beyond the `ARETE.md` excerpt above.
- **Harvest is not a feature-ledger enrichment.** Step 5's `features/<slug>.md` target is a concept that belongs to a repo already mapped by `/ari-map` for ordinary port work; an arete workstream's durable knowledge, if any qualifies, is better captured as a fast-lane backlog pin pointing at the new repo, or folded into a future `/ari-map` run once that repo is mapped in its own right. Do not force an arete workstream into the feature-ledger template just because Step 5 exists.
- **Cutover is not Step 4's job here, ever.** Reconcile ends when the new repo's claims are built and validated — deploy config, DNS, downstream-consumer migration, secrets, and CI remain permanently out of scope, not a "later phase" of this workstream's own reconcile step.
